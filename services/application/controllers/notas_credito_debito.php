...
class Notas_Credito_Debito extends MY_Controller {
    ...
    public function listar_nota_credito() {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');

        $this->load_view('notas_credito_debito/listar_nota_credito', $this->data);
    }

    public function listar_nota_credito_table() {
        ...
        //--- datos ---//
        $notas_credito = $this->app_model->get_notas_credito();
        ...
        if ($notas_credito) {
            foreach ($notas_credito as $key => $value) {
                ...
                $opcion = ' <div class="btn-group">' .
                        ...
                        '<li><a href="' . base_url() . 'notas_credito_debito/modificar_nota_credito/' . $value['idNotaCredito'] . '"><i class="icon-cogs"></i> Editar</a></li>' .
                        '<li><a href="#modal-delete" class="tip deleteNotaCredito" data-id="' . $value['idNotaCredito'] . '" data-toggle="modal" onclick="deleteNotaCredito(' . $value['idNotaCredito'] . ')" ><i class="icon-close"></i> Eliminar</a></li>' .
                        '<li><a onclick="ver_nota_credito(' . $value['idNotaCredito'] . ')"><i style="font-size: 1.5em;" class="fas fa-binoculars"></i> Ver Nota</a></li>' .
                        '</ul>' .
                        '</div>';
    ...
    public function listar_nota_debito() {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');

        $this->load_view('notas_credito_debito/listar_nota_debito', $this->data);
    }

    public function listar_nota_debito_table() {
        //--- Declaracion del arreglo ---//
        $dato = [];
        ...
        $userdata = $this->session->all_userdata();
        //--- datos ---//
        $notas_debito = $this->app_model->get_notas_debito();
        ...
        if ($notas_debito) {
            foreach ($notas_debito as $key => $value) {
                ...
                $opcion = ' <div class="btn-group">' .
                        ...
                        '<button class="btn btn-dark dropdown-toggle" data-toggle="dropdown" style="padding: 3px;font-size: 0.8em;"><span class="caret caret-split"></span></button>' .
                        '<ul class="dropdown-menu icons-right">' .
                        '<li id="editarNotaDebito' . $value['idNotaDebito'] . '" style="display:' . $editar_nota_debito . ';" ><a href="' . base_url() . 'notas_credito_debito/modificar_nota_debito/' . $value['idNotaDebito'] . '"><i class="icon-cogs"></i> Editar</a></li>' .
                        '<li id="eliminarNotaDebito' . $value['idNotaDebito'] . '" style="display:' . $eliminar_no_debito . ';" ><a href="#modal-delete" class="tip deleteNotaDebito" data-id="' . $value['idNotaDebito'] . '" data-toggle="modal" onclick="deleteNotaDebito(' . $value['idNotaDebito'] . ')" ><i class="icon-close"></i> Eliminar</a></li>' .
                        '<li id="abonarNotaDebito' . $value['idNotaDebito'] . '" style="display:' . $abonar_nota_debito . ';" ><a class="tip abonarNotaDebito" data-id="' . $value['idNotaDebito'] . '" data-toggle="modal"onclick="abonar(' . $value['idNotaDebito'] . ')" ><i style="font-size: 1.5em;" class="fas fa-file-invoice-dollar"></i> Abonado</a></li>' .
                        '<li id="verNotaDebito' . $value['idNotaDebito'] . '" style="display:' . $ver_nota_debito . ';" ><a onclick="ver_nota_debito(' . $value['idNotaDebito'] . ')"><i style="font-size: 1.5em;" class="fas fa-binoculars"></i> Ver Nota</a></li>' .
                        '</ul>' .
                        '</div>';
    ...
    public function listar_nota_debito_table_by_idNotaDebito($idNotaDebito) {
        //--- Declaracion del arreglo ---//
        $dato = [];
        ...
        $userdata = $this->session->all_userdata();
        //--- datos ---//
        $notas_debito = $this->app_model->get_notas_debito_by_idNotaDebito($idNotaDebito);
        $notas_debito_detalle = $this->app_model->get_detalle_notas_debito_by_idGenNotaDebito($notas_debito[0]['idGenNotaDebito']);
        ...
        if ($notas_debito_detalle) {
            foreach ($notas_debito_detalle as $key => $value) {
                ...
                $cantidad = '<input type="text" value="' . $value['cantidad'] . '" id="cantProd' . $value['idProducto'] . '" onkeyup="calculoDetalleNotaDebitoModificar(' . $idProducto . ')" class="form-control">' .
                ...
                $descuento = '<div class="input-group">' .
                        '<span class="input-group-addon">%</span>' .
                        '<input type="text" value="' . $value['descuento'] . '" id="descProd' . $value['idProducto'] . '" onkeyup="calculoDetalleNotaDebitoModificar(' . $idProducto . ',' . ')" class="form-control">' .
                        '</div>';
                ...
                $iva = '<select id="selectIva' . $value['idProducto'] . '" class="select-full" onchange="calculoDetalleNotaDebitoModificar(' . $idProducto . ',' . ')" required>' .
                        '<option value="0">IVA</option>' .
                        $iva_tipos_option .
                        '</select>';

                $accion = '<i class="fas fa-trash-alt" style="font-size: 1.6em;padding-right: 5px;" onclick="deleteRowListaNotaDebitoModificar(' . $idProducto . ')"></i>';
    ...
    public function agregar_nota_debito() {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');
        ...
        $this->data['ingresos'] = $this->app_model->get_ingresos();

        $this->data['idGenIngreso'] = "";

        $this->load_view('notas_credito_debito/agregar_nota_debito', $this->data);
    }

    public function agregar_nota_debito_crear_ncnd($idGenIngreso) {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');
        ...
        $this->data['idGenIngreso'] = $idGenIngreso;

        $this->load_view('notas_credito_debito/agregar_nota_debito', $this->data);
    }

    public function set_nota_debito() {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');

        //--- Guardo ---//
        if ($_POST) {
            $datosNotaDebito = $this->input->post('datosNotaDebito', true);
            $datosDetalleNotaDebito = $this->input->post('datosDetalleNotaDebito', true);
...
//                    
                    ...
                    for ($i = 0; $i < count($datosDetalleNotaDebito); $i++) {
                        $result_insert_ingreso_detalle = $this->app_model->insert_nota_debito_detalle(
                        ...
                        $totalIva = $datosDetalleNotaDebito[$i]['subtotalProd'] * $datosDetalleNotaDebito[$i]['iva'];
                    }

                    //--- Guardo - Nota Debito ---//
                    $result_insert_nota_debito = $this->app_model->insert_nota_debito(
                        $idGenNotaDebito, $idVendedor, $datosNotaDebito["selectCliente"], //selectCliente
                        $fechaEmision, //fechaEmision
                        $fechaVencimiento, // fecha Vencimiento
                        $datosNotaDebito["selectTipoNota"], // selectTipoNota
                        $datosNotaDebito["selectVenta"], //selectVenta
                        $totalIva,
                ...
                $result_cuenta_corriente = $this->app_model->insert_cuenta_corriente_idGenNota(
                        $datosNotaDebito["selectVenta"], $idGenNotaDebito, $idGenComprobante = 0, $datosNotaDebito["selectCliente"], //selectCliente
                        $fechaCobroCuentaCorriente, //fechaCobro
                        $debito = $datosNotaDebito["totalVenta"], //Los movimientos que generan la deuda del cliente. Lo que le DEBE a la EMPRESA
                        ...
                        $descripcionCobro = "Nota de Débito"
                );

                //--- Guardo - Historico ---//
                $result_insert_historico = $this->app_model->set_historico(
                        $idVendedor, $idGenNotaDebito, $tipoAccion = 1, $tipoOperacion = 18, $datosNotaDebito["selectCliente"], //detalle
    ...
    public function agregar_nota_credito() {
        ...
        $this->load_view('notas_credito_debito/agregar_nota_credito', $this->data);
    }

    public function agregar_nota_credito_crear_ncnd($idGenIngreso) {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');
    ...
    public function set_nota_credito() {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');

        //--- Guardo ---//
        if ($_POST) {
            $datosNotaCredito = $this->input->post('datosNotaCredito', true);
            $datosDetalleNotaCredito = $this->input->post('datosDetalleNotaCredito', true);
...
//                    ($idGenPresupuesto == "") ? $idGenPresupuesto == "0" : $idGenPresupuesto;
                    ...
                    for ($i = 0; $i < count($datosDetalleNotaCredito); $i++) {
                        $result_insert_ingreso_detalle = $this->app_model->insert_nota_credito_detalle(
                        ...
                        $totalIva += $datosDetalleNotaCredito[$i]['subtotalProd'] * $datosDetalleNotaCredito[$i]['iva'];
                    }

                    //--- Guardo - Nota Credito ---//
                    $result_insert_nota_credito = $this->app_model->insert_nota_credito(
                        $idGenNotaCredito, $idVendedor, $datosNotaCredito["selectCliente"], //selectCliente
                        $fechaEmision, //fechaEmision
                        $fechaVencimiento, // fecha Vencimiento
                        $datosNotaCredito["selectTipoNota"], // selectTipoNota
                        $datosNotaCredito["selectVenta"], //selectVenta
                        $totalIva,
                ...
                $fechaCobroCuentaCorriente = date("Y-m-d H:i:s");
                $result_cuenta_corriente = $this->app_model->insert_cuenta_corriente_idGenNota(
                        $datosNotaCredito["selectVenta"], $idGenNotaCredito, $idGenComprobante = 0, $datosNotaCredito["selectCliente"], //selectCliente
                        $fechaCobroCuentaCorriente, //fechaCobro
                        $debito = 0, //Los movimientos que generan la deuda del cliente. Lo que le DEBE a la EMPRESA
                        $credito = $datosNotaCredito["totalVenta"], //Los movimientos que se ACREDITAN en la EMPRESA.Los pagos que realizan el cliente
                        $idMedioCobro = 1, //Medio de cobro
                        $saldo = $datosNotaCredito["totalVenta"], //Saldo
                        $descripcionCobro = "Nota de Crédito"
                );

                //--- Guardo - Historico ---//
                $result_insert_historico = $this->app_model->set_historico(
                        $idVendedor, $idGenNotaCredito, $tipoAccion = 1, $tipoOperacion = 19, $datosNotaCredito["selectCliente"], //detalle
    ...
    public function listar_nota_credito_table_by_idNotaCredito($idNotaCredito) {
        //--- Declaracion del arreglo ---//
        $dato = [];
        ...
        $userdata = $this->session->all_userdata();
        //--- datos ---//
        $notas_credito = $this->app_model->get_notas_credito_by_idNotaCredito($idNotaCredito);
        $notas_credito_detalle = $this->app_model->get_detalle_notas_credito_by_idGenNotaCredito($notas_credito[0]['idGenNotaCredito']);
        $iva_tipos = $this->app_model->get_iva_tipos();
        $iva_tipos_option = "";
        ...
        if ($notas_credito_detalle) {
            foreach ($notas_credito_detalle as $key => $value) {
                ...
                $cantidad = '<input type="text" value="' . $value['cantidad'] . '" id="cantProd' . $value['idProducto'] . '" onkeyup="calculoDetalleNotaCreditoModificar(' . $idProducto . ')" class="form-control">' .
                        '<div id="errorStock' . $value['idProducto'] . '" class="btn-danger erroBoxs" style="display: none">' .
                        '</div>' . '<input type="hidden" value="' . $value['cantidad'] . '" id="altaProd' . $value['idProducto'] . '" readonly class="form-control">';
                ...
                $descuento = '<div class="input-group">' .
                        '<span class="input-group-addon">%</span>' .
                        '<input type="text" value="' . $value['descuento'] . '" id="descProd' . $value['idProducto'] . '" onkeyup="calculoDetalleNotaCreditoModificar(' . $idProducto . ',' . ')" class="form-control">' .
                        '</div>';
                ...
                $iva = '<select id="selectIva' . $value['idProducto'] . '" class="select-full" onchange="calculoDetalleNotaCreditoModificar(' . $idProducto . ',' . ')" required>' .
                        '<option value="0">IVA</option>' .
                        $iva_tipos_option .
                        '</select>';

                $accion = '<i class="fas fa-trash-alt" style="font-size: 1.6em;padding-right: 5px;" onclick="deleteRowListaNotaCreditoModificar(' . $idProducto . ')"></i>';
    ...
    public function modificar_nota_debito($idNotaDebito = null) {
        ...
        $nota_debito = $this->app_model->get_notas_debito_by_idNotaDebito($idNotaDebito);
        ...
        $this->load_view('notas_credito_debito/modificar_nota_debito', $this->data);
    }

    public function update_nota_debito() {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');
        ...
        if ($_POST) {
            ...
            $nota_debito_anterior = $this->app_model->get_notas_debito_by_idNotaDebito($idNotaDebito);
            $idGenNotaDebito = $nota_debito_anterior[0]['idGenNotaDebito'];
            $nota_debito_detalle_anterior = $this->app_model->get_detalle_notas_debito_by_idGenNotaDebito($idGenNotaDebito);
                ...
                if (
                    ...
                    if (empty($datosDetalleNotaDebito)) {
                        ...
                        foreach ($datosDetalleNotaDebito as $keys => $values) {
                            $result_insert_nota_debito_detalle = $this->app_model->insert_nota_debito_detalle(
                        ...
                        foreach ($nota_debito_detalle_anterior as $key => $value) {
                            if (!in_array($nota_debito_detalle_anterior[$key]['idProducto'], $valorComparar)) {
                                //--- Borrar ---//
                                $delete_ingreso = $this->app_model->drop_nota_debito_detalle($idGenNotaDebito, $nota_debito_detalle_anterior[$key]['idProducto']);
                            }
                        }
                    ...
                    $this->app_model->update_cuenta_corrientes_by_idGenNota_ordenAsc_limit1($idGenNotaDebito, $datosNotaDebito["selectCliente"], $datosNotaDebito["totalVenta"], 0, $datosNotaDebito["totalVenta"]);

                    //--- Guardo - Historico ---//
                    $resultHistorico = $this->app_model->set_historico(
                            $idVendedor, $idGenNotaDebito, $tipoAccion = 2, $tipoOperacion = 18, $datosNotaDebito["selectCliente"], //detalle
                            $datosNotaDebito["totalVenta"] //total
                    );
                }

                //--- Guardo - Nota Debito ---//
                $result_insert_nota_debito = $this->app_model->update_nota_debito(
                    $idGenNotaDebito, $idVendedor, $datosNotaDebito["selectCliente"], //selectCliente
                    $fechaEmision, //fechaEmision
                    $fechaVencimiento, // fecha Vencimiento
                    $datosNotaDebito["selectTipoNota"], // selectTipoNota
    ...
    public function delete_nota_debito() {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');
        ...
        $nota_debito_detalle = $this->app_model->get_detalle_notas_debito_by_idGenNotaDebito($idGenNotaDebito);
        ...
        if ($_POST) {
            ...
            $this->app_model->drop_nota_debito($idGenNotaDebito);

            //--- Borramos el detale de la nota de debito ---//
            foreach ($nota_debito_detalle as $key => $value) {
                $this->app_model->drop_nota_debito_detalle($idGenNotaDebito, $value['idProducto']);
            }

            //--- Actualizamos la cuenta corriente ---//
            $this->app_model->drop_cuenta_corrientes_by_idGenNota($idGenNotaDebito);

            //--- Guardo - Historico ---//
            $resultHistorico = $this->app_model->set_historico(
                    $idVendedor, $idGenNotaDebito, $tipoAccion = 3, $tipoOperacion = 18, $nota_debito[0]['idCliente'], //detalle
    ...
    public function abonar_nota_debito() {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');
        ...
        if ($_POST) {
            ...
            $abonado = $this->app_model->abonar_nota_debito($idNotaDebito);
    ...
    public function modificar_nota_credito($idNotaCredito = null) {
        ...
        $nota_credito = $this->app_model->get_notas_credito_by_idNotaCredito($idNotaCredito);
        ...
        $this->load_view('notas_credito_debito/modificar_nota_credito', $this->data);
    }

    public function update_nota_credito() {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');
        ...
        if ($_POST) {
            ...
            $nota_credito_anterior = $this->app_model->get_notas_credito_by_idNotaCredito($idNotaCredito);
            $idGenNotaCredito = $nota_credito_anterior[0]['idGenNotaCredito'];
            $nota_credito_detalle_anterior = $this->app_model->get_detalle_notas_credito_by_idGenNotaCredito($idGenNotaCredito);
                ...
                if (
                    ...
                    if (empty($datosDetalleNotaCredito)) {
                        ...
                        foreach ($datosDetalleNotaCredito as $keys => $values) {
                            $result_insert_nota_credito_detalle = $this->app_model->insert_nota_credito_detalle(
                        ...
                        foreach ($nota_credito_detalle_anterior as $key => $value) {
                            if (!in_array($nota_credito_detalle_anterior[$key]['idProducto'], $valorComparar)) {
                                //--- Borrar ---//
                                $delete_ingreso = $this->app_model->drop_nota_credito_detalle($idGenNotaCredito, $nota_credito_detalle_anterior[$key]['idProducto']);
                            }
                        }
                    ...
                    //--- Actualizamos la cuenta corriente ---//
                    $this->app_model->update_cuenta_corrientes_by_idGenNota_ordenAsc_limit1($idGenNotaCredito, $datosNotaCredito["selectCliente"], $datosNotaCredito["totalVenta"], 0, $datosNotaCredito["totalVenta"]);

                    //--- Guardo - Historico ---//
                    $resultHistorico = $this->app_model->set_historico(
                            $idVendedor, $idGenNotaCredito, $tipoAccion = 2, $tipoOperacion = 19, $datosNotaCredito["selectCliente"], //detalle
                            $datosNotaCredito["totalVenta"] //total
                    );
                }

                //--- Guardo - Nota Credito ---//
                $result_insert_nota_credito = $this->app_model->update_nota_credito(
                    $idGenNotaCredito, $idVendedor, $datosNotaCredito["selectCliente"], //selectCliente
                    $fechaEmision, //fechaEmision
                    $fechaVencimiento, // fecha Vencimiento
                    $datosNotaCredito["selectTipoNota"], // selectTipoNota
    ...
    public function delete_nota_credito() {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');
        ...
        $nota_credito_detalle = $this->app_model->get_detalle_notas_credito_by_idGenNotaCredito($idGenNotaCredito);
        ...
        if ($_POST) {
            ...
            //--- Borramos la nota de debito ---//
            $this->app_model->drop_nota_credito($idGenNotaCredito);

            //--- Borramos el detale de la nota de debito ---//
            foreach ($nota_credito_detalle as $key => $value) {
                $this->app_model->drop_nota_credito_detalle($idGenNotaCredito, $value['idProducto']);
            }

            //--- Actualizamos la cuenta corriente ---//
            $this->app_model->drop_cuenta_corrientes_by_idGenNota($idGenNotaCredito);

            //--- Guardo - Historico ---//
            $resultHistorico = $this->app_model->set_historico(
                    $idVendedor, $idGenNotaCredito, $tipoAccion = 3, $tipoOperacion = 18, $nota_credito[0]['idCliente'], //detalle
    ...
    public function generar_pdf_nota_debito() {

        $idNotaDebito = $this->input->post('idNota', true);
        
        //--- Datos ---//
        $nota_debito = $this->app_model->get_notas_debito_by_idNotaDebito($idNotaDebito);
        $nota_debito_detalle = $this->app_model->get_detalle_notas_debito_by_idGenNotaDebito($nota_debito[0]['idGenNotaDebito']);
        $ingreso_nota_debito = $this->app_model->get_ingresos_nota_debito($idNotaDebito);

        //-- CARGO LIBRERIAS --//
        $this->load->library('html2pdf');

        $this->html2pdf->folder('./uploads/notasdebito/');
        ...
        $this->html2pdf->html('
                    ...
                    <div style="border: 0.5px solid black;margin: 0 auto;position: absolute;width: 100%;border: 0.5px solid black;">
                        <table class="tg" style="width: 100%;" >
                            ...
                            <tr>
                                <td class="tg-zb0m" colspan="3"><span style="font-weight:bold">Empresa</span></td>
                            ...
                            <tr>
                                <td class="tg-704r" colspan="4">Razón Social: ' . ($ingreso_nota_debito[0]['razonSocial'] == "" ? "-" : $ingreso_nota_debito[0]['razonSocial']) . '<br><br>Domicilio Comercial: ' . ($ingreso_nota_debito[0]['domicilioComercial'] == "" ? "-" : $ingreso_nota_debito[0]['domicilioComercial']) .'<br><br>Condición frente al IVA: '. $ingreso_nota_debito[0]['condicionIva'] .'</td>
                                <td class="tg-704r" colspan="4">Nro. Factura de referencia: '. ($ingreso_nota_debito[0]['idIngreso'] == "" ? "-" : $ingreso_nota_debito[0]['idIngreso']) .'<br>Punto de Venta: - Comp. Nro: '. ($nota_debito[0]['idNotaDebito'] == "" ? "-" : $nota_debito[0]['idNotaDebito']) .'<br>Fecha de Emisión: '. ($nota_debito[0]['fechaAlta'] == "" ? "-" : $nota_debito[0]['fechaAlta']) .'<br>CUIT: '. ($ingreso_nota_debito[0]['cuit'] == "" ? "-" : $ingreso_nota_debito[0]['cuit']) .'<br>Ingresos Brutos: - <br>Fecha de Inicio de Actividades: - </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td class="tg-704r" colspan="4">CUIT: '. ($ingreso_nota_debito[0]['cuit'] == "" ? "-" : $ingreso_nota_debito[0]['cuit']) .'<br>Condición frente al IVA:'. $ingreso_nota_debito[0]['condicionIva'] .'<br>Condición de venta: Efectivo</td>
                                <td class="tg-704r" colspan="4">Apellido y nombre / Razón Social: '. ($ingreso_nota_debito[0]['apellidoCliente'] == "" ? "-" : $ingreso_nota_debito[0]['apellidoCliente'] . ", ") .''. ($ingreso_nota_debito[0]['nombreCliente'] == "" ? "" : $ingreso_nota_debito[0]['nombreCliente']) .'<br>Domicilio: '. ($ingreso_nota_debito[0]['domicilioCliente'] == "" ? "-" : $ingreso_nota_debito[0]['domicilioCliente']) .'</td>
                            </tr>
                            <tr>
    ...
    public function generar_pdf_nota_credito() {

        $idNotaCredito = $this->input->post('idNota', true);
        
        //--- Datos ---//
        $nota_credito = $this->app_model->get_notas_credito_by_idNotaCredito($idNotaCredito);
        $nota_credito_detalle = $this->app_model->get_detalle_notas_credito_by_idGenNotaCredito($nota_credito[0]['idGenNotaCredito']);
        $ingreso_nota_credito = $this->app_model->get_ingresos_nota_credito($idNotaCredito);

        //-- CARGO LIBRERIAS --//
        $this->load->library('html2pdf');

        $this->html2pdf->folder('./uploads/notascredito/');
        ...
        $this->html2pdf->html('
                    ...
                    <div style="border: 0.5px solid black;margin: 0 auto;position: absolute;width: 100%;border: 0.5px solid black;">
                        <table class="tg" style="width: 100%;" >
                            ...
                            <tr>
                                <td class="tg-704r" colspan="4">Razón Social: ' . ($ingreso_nota_credito[0]['razonSocial'] == "" ? "-" : $ingreso_nota_credito[0]['razonSocial']) . '<br><br>Domicilio Comercial: ' . ($ingreso_nota_credito[0]['domicilioComercial'] == "" ? "-" : $ingreso_nota_credito[0]['domicilioComercial']) .'<br><br>Condición frente al IVA: '. $ingreso_nota_credito[0]['condicionIva'] .'</td>
                                <td class="tg-704r" colspan="4">Nro. Factura de referencia: '. ($ingreso_nota_credito[0]['idIngreso'] == "" ? "-" : $ingreso_nota_credito[0]['idIngreso']) .'<br>Punto de Venta: - Comp. Nro:'. ($nota_credito[0]['idNotaCredito'] == "" ? "-" : $nota_credito[0]['idNotaCredito']) .'<br>Fecha de Emisión: '. ($nota_credito[0]['fechaAlta'] == "" ? "-" : $nota_credito[0]['fechaAlta']) .'<br>CUIT: '. ($ingreso_nota_credito[0]['cuit'] == "" ? "-" : $ingreso_nota_credito[0]['cuit']) .'<br>Ingresos Brutos: - <br>Fecha de Inicio de Actividades: - </td>
                            </tr>
                            <tr>
                                <td class="tg-nkla" colspan="3">Periodo Facturado Desde: - </td>
                                <td class="tg-nkla" colspan="2"><span style="font-weight:400;font-style:normal">Hasta: - </span></td>
                            ...
                            <tr>
                                <td class="tg-704r" colspan="4">CUIT: '. ($ingreso_nota_credito[0]['cuit'] == "" ? "-" : $ingreso_nota_credito[0]['cuit']) .'<br>Condición frente al IVA: '. $ingreso_nota_credito[0]['condicionIva'] .'<br>Condición de venta: Efectivo</td>
                                <td class="tg-704r" colspan="4">Apellido y nombre / Razón Social: '. ($ingreso_nota_credito[0]['apellidoCliente'] == "" ? "-" : $ingreso_nota_credito[0]['apellidoCliente'] . ", ") .''. ($ingreso_nota_credito[0]['nombreCliente'] == "" ? "" : $ingreso_nota_credito[0]['nombreCliente']) .'<br>Domicilio: '. ($ingreso_nota_credito[0]['domicilioCliente'] == "" ? "-" : $ingreso_nota_credito[0]['domicilioCliente']) .'</td>
                            </tr>
                            <tr>
                                <td class="tg-r0gd">Código</td>
                                <td class="tg-r0gd">Producto / Servicio</td>
    ...
    public function nota_credito_debito_venta($idGenIngreso) {
        header("Access-Control-Allow-Origin: *");
        header('Access-Control-Allow-Credentials: true');
        ...
        $this->data['idGenIngreso'] = $idGenIngreso;
        $this->data['idGenIngresoComillas'] = "'" . $idGenIngreso . "'";
        $this->data['idIngreso'] = $ingreso[0]['idIngreso'];

        //--- Tablas ---//
        $estadoCuenta = $this->app_model->get_estado_cuenta_by_idGenIngreso_ncnd($idGenIngreso);
        $this->data['estadoCuenta'] = $estadoCuenta;

        $notas_credito = $this->app_model->get_notas_credito_by_idGenIngreso($idGenIngreso);
        $total_nc = 0;
        if($notas_credito) {
            foreach($notas_credito as $key => $value) {
                $total_nc += $value['total'];
            }
        }
        $this->data['notas_credito'] = $notas_credito;

        $notas_debito = $this->app_model->get_notas_debito_by_idGenIngreso($idGenIngreso);
        $total_nd = 0;
        if($notas_debito) {
            foreach($notas_debito as $key => $value) {
                $total_nd += $value['total'];
            }
        }
        $this->data['notas_debito'] = $notas_debito;

        $this->data['remitos'] = $this->app_model->get_remitos_by_idGenIngreso($idGenIngreso);
        
        $this->data['remitos_sin_transportistas'] = $this->app_model->get_remitos_by_idGenIngreso_sin_transportistas($idGenIngreso);

        //--- Valores totales ---//
        $this->data['total'] = $ingreso[0]['total'];
        $this->data['nc'] = $total_nc;
        $this->data['nd'] = $total_nd;
        $this->data['cobrado'] = $ingreso[0]['total'] - $ingreso[0]['aCobrar'];
        $this->data['aCobrar'] = $ingreso[0]['aCobrar'];


        $this->load_view('notas_credito_debito/nota_credito_debito_venta', $this->data);
    }
