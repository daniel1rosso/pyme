    ...
    public function listar_saldos() {
        ...
        if($disponibleCajas){
            foreach ($disponibleCajas as $key => $value) {
                ...
                $cuentas = $this->app_model->get_estado_caja_by_idCaja($value['idCuenta']);
                if($cuentas){
                    foreach ($cuentas as $keyCuenta => $value) {
                        $ingreso += $value['ingreso'];
                        $egreso += $value['egreso'];
                    }	
                    $totalCuenta = $ingreso - $egreso;  
                    $this->data['disponibleCajas'][$key]['totalCuenta']      = $totalCuenta;
                }
...
//        print_r($this->data['disponibleBancos']);
    ...
    public function get_movimientos_caja_by_fecha(){
        header("Access-Control-Allow-Origin: *");  
        header('Access-Control-Allow-Credentials: true');  
        
        $selectMedioCajaFiltro = $this->input->post('selectMedioCajaFiltro', true);
        $fechaDesde = $this->input->post('fechaDesde', true);
        $fechaHasta = $this->input->post('fechaHasta', true);
        $msg="";
        
        if (!empty($fechaDesde) AND !empty($fechaHasta) AND !empty($selectMedioCajaFiltro)) {
            $movimientosCaja = $this->app_model->get_movimientos_caja_by_fecha($fechaDesde,$fechaHasta,$selectMedioCajaFiltro);
            
            if($movimientosCaja){
                $this->data['movimientosCaja']= $movimientosCaja;
                $ingreso=0;
                $egreso=0;
                foreach ($movimientosCaja as $keyCuenta => $value) {
                    $ingreso+= $value['ingreso'];
                    $egreso+= $value['egreso'];
                }
                
                $resultados= count($movimientosCaja);
                $saldo=$ingreso-$egreso;
                
                $msg="Ok";
                $dato = array("valid"=>true,"msg"=>$msg,"movimientosCaja"=>$movimientosCaja,"saldo"=>$saldo,"resultados"=>$resultados);                   
            }else{
                $msg="No hay registros en las fechas ingresadas";
                $dato = array("valid"=>false,"msg"=>$msg);    
            }
        }else{
            $msg="Debe completar los 3 campos";
            $dato = array("valid"=>false,"msg"=>$msg);    
        }
        ...
        if($_POST) {
            ...
            if(
                ...
                if(
                    ...
                    $cuenta = $this->app_model->get_cuenta_tesoreria_byIdGen($idGenCuenta);
                    if($cuenta){
                        $msg="Cuenta de tesoreria agregada con exito";
                        $dato = array("valid"=>true,"msg"=>$msg,"cuenta"=>$cuenta);             
                    }  
                }else{
                    $msg="Error al agregar la cuenta de tesoreria, vuelva a intentarlo";
                    $dato = array("valid"=>false,"msg"=>$msg);                
                }              
            }
        }else{
    ...
    public function set_movimiento() {
        ...
        if ($_POST) {
            ...
            if(
                !empty($montoMovimiento) AND            
                !empty($selectCuentaSalida) AND            
                !empty($selectCuentaEntrada)
            ) {
                $userdata = $this->session->all_userdata();
                ...
                if($saldoCaja > 0 && $saldoCaja > $montoMovimiento){
                    ...
                    //--- Guardo - Historico ---//
                    $result_insert_historico = $this->app_model->set_historico(
                            $idVendedor, $idGenIngreso=0, $tipoAccion = 1, $tipoOperacion = 13, $selectCuentaSalida, //detalle
                            $montoMovimiento //total
                    );
                    $result_insert_historico = $this->app_model->set_historico(
                            $idVendedor, $idGenIngreso=0, $tipoAccion = 1, $tipoOperacion = 14, $selectCuentaEntrada, //detalle
                            $montoMovimiento //total
                    );
                    if ($result) {
        ...
        if($_POST) {
            ...
            if(
                ...
                if(
                    ...
                    if($cuenta){
                        $msg="Cuenta tesoreria fue actualizada con exito";
                        $dato = array("valid"=>true,"msg"=>$msg,"cuenta"=>$cuenta);             
                    }  
                }else{
                    $msg="Error al actualizar la cuenta de tesoreria, vuelva a intentarlo";
                    $dato = array("valid"=>false,"msg"=>$msg);                
                }              
            }
        }else{
    ...
    public function eliminar_cuenta_tesoreria() {
        ...
        if (!empty($idGenCuenta)) {
            ...
            $tesoreria_cuenta = $this->app_model->get_cuenta_tesoreria_byIdGenCuenta($idGenCuenta);
            $cuenta_movimientos = $this->app_model->get_estado_caja_by_idCaja($tesoreria_cuenta[0]['idCuenta']);
            if (!$cuenta_movimientos) {
                $result_eliminar_cuenta = $this->app_model->eliminar_cuenta_tesoreria($idGenCuenta);
            } else {
                $result_eliminar_cuenta = false;
                $existe = 1;
            }	
            ...
            if(
                ...
                $msg="Cuenta tesoreria fue eliminada con exito";
                ...
                $msg="Error al eliminar la cuenta de tesoreria porque contiene movimientos asociados, vuelva a intentarlo";
                $dato = array("valid"=>false,"msg"=>$msg);    
            } else {
                $msg="Error al eliminar la cuenta de tesoreria, vuelva a intentarlo";
                $dato = array("valid"=>false,"msg"=>$msg);    
            }
        }else{
            $msg="No hay post";
